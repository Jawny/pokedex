...
export default class Pokemon extends Component {
  ...
  async componentDidMount() {
    ...
    pokemonRes.data.stats.map(pokemonStat => {
      switch (pokemonStat.stat.name) {
        ...
        case "speed":
          ...
          break;
        case "special-attack":
          specialAttack = pokemonStat["base_stat"];
          break;
        case "special-defense":
          specialDefense = pokemonStat["base_stat"];
          break;
      }
    });
    ...
    const abilities = pokemonRes.data.abilities.map(pokemonAbility => {
    ...
    const evs = pokemonRes.data.stats
      ...
      .map(pokemonStat => {
        return `${pokemonStat.effort} ${pokemonStat.stat.name
          .toLowerCase()
          .split("-")
          .map(s => s.charAt(0).toUpperCase() + s.substring(1))
          .join(" ")}`;
              ...
              <div className="col-7">
                <div className="float-right">
                  {this.state.types.map(type => (
                    <span
                      key={type}
                      className="badge-primary badge-pill mr-1"
                      style={{
